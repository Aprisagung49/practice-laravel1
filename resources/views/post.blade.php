    ...
    {{-- <article class ="py-8 max-w-screen-md">
        ...
        <div>
            ...
            <a href="/categories/{{ $post->category->slug }}" class=" text-base text-gray-500 hover:underline">{{ $post->category->name }}</a> |
             {{$post->created_at->diffForHumans()}} {{-- diffForHumans() digunakan untuk format 1 minutes ago --}}
        {{-- </div>
        <p class="my-4 font-light">{{ $post['body']}}     {{-- Str::limit() digunakan untuk membatasi Jumlah Karakter --}}
        {{-- </p>
        <a href="/posts" class="text-medium text-blue-900 hover:underline">&laquo; Back To Post </a> --}}
        {{-- &laquo; digunakan untuk menampilkan simbol << --}}
    {{-- </article> --}} 
